...
const POS = () => {
    ...
    // Optimized filtering and sorting (HARD-LOCKED to 3 Beers as requested)
    const filteredProducts = useMemo(() => {
        ...
        // 🎯 STRICT FILTER: Only show these 3 exact items
        let result = products.filter(p => 
            targetNames.some(name => (p.name || '').includes(name))
        );

        // Sorting (Match the order of targetNames)
        result.sort((a, b) => {
            const indexA = targetNames.findIndex(name => (a.name || '').includes(name));
            const indexB = targetNames.findIndex(name => (b.name || '').includes(name));
